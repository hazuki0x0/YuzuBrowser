...
public class BrowserHistoryFragment extends Fragment implements BrowserHistoryAdapter.OnHistoryRecyclerListener, ActionMode.Callback {
    ...
    public boolean onRecyclerItemLongClicked(View v, final int position) {
        if (!pickMode) {
            ...
            menu.add(R.string.open).setOnMenuItemClickListener(item -> {
                sendUrl(url, BrowserManager.LOAD_URL_TAB_CURRENT);
                return false;
            });
            menu.add(R.string.open_new).setOnMenuItemClickListener(item -> {
                sendUrl(url, BrowserManager.LOAD_URL_TAB_NEW);
                return false;
            });
            menu.add(R.string.open_bg).setOnMenuItemClickListener(item -> {
                sendUrl(url, BrowserManager.LOAD_URL_TAB_BG);
                return false;
            });
            menu.add(R.string.open_new_right).setOnMenuItemClickListener(item -> {
                sendUrl(url, BrowserManager.LOAD_URL_TAB_NEW_RIGHT);
                return false;
            });
            menu.add(R.string.open_bg_right).setOnMenuItemClickListener(item -> {
                sendUrl(url, BrowserManager.LOAD_URL_TAB_BG_RIGHT);
                return false;
            });
            menu.add(R.string.add_bookmark).setOnMenuItemClickListener(item -> {
                AddBookmarkKt.showDialog(getActivity(), getChildFragmentManager(), title, url);
                return false;
            });
            menu.add(R.string.delete_history).setOnMenuItemClickListener(item -> {
