...
package jp.hazuki.yuzubrowser.legacy.download.service
...
import jp.hazuki.yuzubrowser.legacy.download.core.utils.getNotificationString
import jp.hazuki.yuzubrowser.legacy.download.core.utils.registerMediaScanner
import jp.hazuki.yuzubrowser.legacy.download.core.utils.toDocumentFile
import jp.hazuki.yuzubrowser.legacy.download.service.connection.ServiceClient
...
class DownloadService : DaggerService(), ServiceClient.ServiceClientListener {
    ...
    override fun onCreate() {
        ...
        val filter = IntentFilter(Constants.intent.ACTION_CANCEL_DOWNLOAD)
        filter.addAction(Constants.intent.ACTION_PAUSE_DOWNLOAD)

        val signature = browserApplicationContext.permissionAppSignature
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent != null) {
            ...
            when (intent.action) {
                Constants.intent.ACTION_START_DOWNLOAD -> {
                    val root = intent.getParcelableExtra<Uri>(Constants.intent.EXTRA_ROOT_URI).toDocumentFile(this)
                    val file = intent.getParcelableExtra<DownloadFile>(Constants.intent.EXTRA_REQUEST_DOWNLOAD)
                    val metadata = intent.getParcelableExtra<MetaData?>(Constants.intent.EXTRA_METADATA)
                    if (file != null) {
                        thread = FirstDownloadThread(root, file, metadata)
                    }
                }
                Constants.intent.ACTION_RESTART_DOWNLOAD -> {
                    val id = intent.getLongExtra(Constants.intent.EXTRA_ID, -1)
    ...
    private val notificationControl = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent?) {
            ...
            if (id >= 0) {
                when (intent.action) {
                    Constants.intent.ACTION_CANCEL_DOWNLOAD -> cancelDownload(id)
                    Constants.intent.ACTION_PAUSE_DOWNLOAD -> pauseDownload(id)
                }
            }
        }
    }
    ...
    private abstract inner class DownloadThread(private val request: DownloadRequest) : Thread(), Downloader.DownloadListener {
        ...
        override fun onFileDownloadAbort(info: DownloadFileInfo) {
            ...
            if (info.state == DownloadFileInfo.STATE_PAUSED) {
                NotificationCompat.Builder(this@DownloadService, Constants.notification.CHANNEL_DOWNLOAD_NOTIFY).run {
                    ...
                    val resume = Intent(Constants.intent.ACTION_RESTART_DOWNLOAD).apply { putExtra(Constants.intent.EXTRA_ID, info.id) }
        ...
        private fun setAction(info: DownloadFileInfo) {
            if (!isActionAdded) {
                ...
                notification.run {
                    if (info.resumable) {
                        val pause = Intent(Constants.intent.ACTION_PAUSE_DOWNLOAD).apply { putExtra(Constants.intent.EXTRA_ID, info.id) }
                    ...
                    val cancel = Intent(Constants.intent.ACTION_CANCEL_DOWNLOAD).apply { putExtra(Constants.intent.EXTRA_ID, info.id) }
