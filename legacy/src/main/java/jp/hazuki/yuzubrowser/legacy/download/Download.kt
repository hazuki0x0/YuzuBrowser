...
fun Context.download(root: Uri, file: DownloadFile, meta: MetaData?) {
    val intent = Intent(this, DownloadService::class.java).apply {
        action = Constants.intent.ACTION_START_DOWNLOAD
        putExtra(Constants.intent.EXTRA_ROOT_URI, root)
        putExtra(Constants.intent.EXTRA_REQUEST_DOWNLOAD, file)
        putExtra(Constants.intent.EXTRA_METADATA, meta)
    }
    startService(intent)
}
...
fun Context.reDownload(id: Long) {
    val intent = Intent(this, DownloadService::class.java).apply {
        action = Constants.intent.ACTION_RESTART_DOWNLOAD
        putExtra(Constants.intent.EXTRA_ID, id)
    }
    startService(intent)
}